...
//   const storageKey = `@profileImage_${employeeData?.employee_name}`;
...
//         Alert.alert('Session Error', 'Could not load session. Please login again.');
//       }
//     };
...
//               message: 'App needs access to your images to upload profile photos.',
...
//               message: 'This app needs access to your photo gallery to upload profile images.',
//               buttonNeutral: 'Ask Me Later',
//               buttonNegative: 'Cancel',
//               buttonPositive: 'OK',
//             }
...
//       if (json?.message?.file_url && json?.message?.name) {
//         return {
//           file_url: json.message.file_url,
//           file_name: json.message.name,
//         };
//       } else {
//         throw new Error('Upload failed: Missing file_url or name');
//       }
//     } catch (error) {
//       console.error('Upload error:', error);
//       Alert.alert('Upload Error', 'Failed to upload image to ERPNext.');
//       return null;
//     }
//   };

//   const updateEmployeePhoto = async (fileUrl) => {
//     const apiUrl = `${ERP_BASE_URL}/api/resource/Employee/${employeeData.name}`;
//     try {
...
//           Cookie: `sid=${sid}`,
//         },
//         body: JSON.stringify({ image: fileUrl }), // ✅ Correct field
//       });

//       if (!res.ok) {
//         const json = await res.json();
//         throw new Error(json.message || 'Failed to update employee image');
//       }
//       return true;
//     } catch (error) {
//       console.error('Update employee image error:', error);
//       Alert.alert('Update Error', 'Failed to update employee image in ERPNext.');
//       return false;
//     }
//   };
...
//       const uploadResult = await uploadImageFileToERP(selectedUri);

//       if (uploadResult) {
//         const updated = await updateEmployeePhoto(uploadResult.file_url);
...
//               ? imageUri
//               : employeeData?.image
//               ? `${ERP_BASE_URL}${employeeData.image}?t=${Date.now()}`
...
//       <Text style={styles.name}>{employeeData?.employee_name || 'N/A'}</Text>

//       <View style={styles.infoBox}>

         
//          <Text style={{fontSize:20, textAlign:'center', fontWeight:'bold', color:'black', marginBottom:20}} > Employee Detail</Text>
//         <Text style={styles.label}>Employee ID:    <Text style={styles.value}>{employeeData?.name || 'N/A'}</Text></Text>
       
//         <Text style={styles.label}>Gender:   <Text style={styles.value}>{employeeData?.gender || 'N/A'}</Text></Text>
        
//         <Text style={styles.label}>Designation:  <Text style={styles.value}>{employeeData?.designation || 'N/A'}</Text></Text>
        
//         <Text style={styles.label}>Department:  <Text style={styles.value}>{employeeData?.department || 'N/A'}</Text></Text>
        
//         <Text style={styles.label}>Date of Joining:  <Text style={styles.value}>{employeeData?.date_of_joining || 'N/A'}</Text></Text>


//         <Text style={styles.label}>Date of Birth:  <Text style={styles.value}>{employeeData?.date_of_birth || 'N/A'}</Text></Text>
//       </View>
//     </View>
//   );
// };
...
//   container: { flex: 1, alignItems: 'center', paddingTop: 40, backgroundColor: '#f1f3f6' },
//   profileImage: {
//     width: 120,
//     height: 120,
//     borderRadius: 60,
...
//     borderWidth: 2,
//   },
//   name: { fontSize: 22, fontWeight: 'bold', marginBottom: 20, color: '#333' },
//   infoBox: {
//     width: '85%',
//     backgroundColor: '#fff',
//     padding: 20,
...
//     elevation: 3,
//   },
//   label: { fontSize: 16, color: '#666', marginTop: 10, fontWeight: '600' },
...
//           message: 'App needs access to your gallery to upload profile photos',
//           buttonPositive: 'OK',
//           buttonNegative: 'Cancel',
//         });
...
//     if (!employeeData?.image || !sid) return true;
    
//     try {
//       const fileName = employeeData.image.split('/').pop();
//       const deleteUrl = `${ERP_BASE_URL}/api/method/erpnext.www.delete_file.delete_file`;
...
//       return true;
//     } catch (error) {
//       console.error('Failed to remove old profile image:', error);
...
//       if (response.respInfo.status === 200 && json.message) {
//         return {
//           file_url: json.message.file_url,
//           file_name: json.message.name,
//         };
...
//       // Update the employee record with new image URL
...
//         <Image
//           source={{
//             uri: imageUri || 'https://via.placeholder.com/120',
//           }}
//           style={styles.profileImage}
//           resizeMode="cover"
//         />
...
//       <View style={styles.infoBox}>
//         <Text style={styles.infoTitle}>Employee Detail</Text>
//         <Text style={styles.label}>Employee ID: <Text style={styles.value}>{employeeData?.name || 'N/A'}</Text></Text>
//         <Text style={styles.label}>Gender: <Text style={styles.value}>{employeeData?.gender || 'N/A'}</Text></Text>
//         <Text style={styles.label}>Designation: <Text style={styles.value}>{employeeData?.designation || 'N/A'}</Text></Text>
//         <Text style={styles.label}>Department: <Text style={styles.value}>{employeeData?.department || 'N/A'}</Text></Text>
